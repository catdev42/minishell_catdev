...
/*   Updated: 2024/07/14 15:52:39 by myakoven         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	getlen_command(t_tools *tools, int i)
{
	...
	while (tools->line[i])
	{
		...
		else if (!istoken(tools->line[i]) && !isquote(tools->line[i]))
			i++;
		else if (istoken(tools->line[i]))
			break ;
	}
	return (i - og_index); // change deleted "+1"
}
...
// FIX this with skip space! and fix SPACES
